...
def connect_state_db(db_path: Path, reset: bool) -> sqlite3.Connection:
    ...
    conn.execute("PRAGMA mmap_size=268435456;")  # 256 MiB memory-mapped I/O, if supported
...
def spend_input(
    ...
    try:
        row = conn.execute(
            ...
            WHERE txid = ? AND vout = ?
            RETURNING value_sats, created_time;
            """,
            (prev_txid, prev_vout),
        ).fetchone()
        ...
        raise RuntimeError(
            "This optimized version of spend_input() requires SQLite support "
            "for DELETE ... RETURNING, available in SQLite 3.35.0 and later. "
            "Your Python sqlite3 module appears not to support it. "
            "Use the previous SELECT plus DELETE implementation, or update SQLite."
        ) from exc
