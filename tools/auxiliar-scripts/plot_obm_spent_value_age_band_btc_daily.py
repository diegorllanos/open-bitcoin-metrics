...
Generate a stacked area plot from the OBM wide daily age-band spent-value CSV:
...
contribution of each age band to total daily spent output value.
"""
...
EXPECTED_SERIES_ID = "obm_spent_value_age_band_btc_daily"
EXPECTED_UNIT = "BTC"
...
def read_age_band_csv(path: Path) -> Tuple[List[date], Dict[str, List[Decimal]], str, str]:
    if not path.exists():
    ...
    # Ensure chronological order even if the CSV was not sorted.
...
def plot_stacked_area(
    ...
    *,
    ...
    ax.set_title(
        "Open Bitcoin Metrics: Spent Value by Age Band\n"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "csv_file",
        type=Path,
        help="Path to obm_spent_value_age_band_btc_daily CSV file.",
    )
...
def main() -> int:
    ...
    try:
        dates, values_by_band, release_version, date_range_label = read_age_band_csv(
            args.csv_file
        )
        ...
        print(f"Wrote stacked age-band plot to {args.output}")
