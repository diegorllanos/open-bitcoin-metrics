...
Generate the Open Bitcoin Metrics daily transaction-count series:

    obm_tx_count_daily

The script queries a local Bitcoin Core node through JSON-RPC, counts the
number of transactions confirmed in blocks whose timestamps fall within the
requested UTC date interval, and writes a standardized OBM CSV file.
...
def build_rpc_config(args: argparse.Namespace) -> BitcoinRPCConfig:
    ...
    if rpc_user is None or rpc_password is None:
        ...
        cookie_path = Path(args.cookie_path).expanduser() if args.cookie_path else datadir / ".cookie"
...
def find_first_height_at_or_after_timestamp(
    ...
    expanded backward by --height_margin before the actual scan. This makes the
    search efficient while preserving practical safety for daily aggregation.
    """
    low = 0
    high = best_height
...
def find_last_height_at_or_before_timestamp(
    ...
    The result is later expanded forward by --height_margin.
    """
    low = 0
    high = best_height
...
def get_block_tx_count(rpc: BitcoinRPC, height: int) -> tuple[int, int]:
    """
    Return (block_timestamp, transaction_count) for a block height.

    Bitcoin Core usually returns 'nTx' in the decoded block object. As a
    fallback, count the length of the 'tx' array when available.
    """
    block_hash = rpc.call("getblockhash", [height])
    block = rpc.call("getblock", [block_hash, 1])
    ...
    if "nTx" in block:
        tx_count = int(block["nTx"])
    elif "tx" in block:
        tx_count = len(block["tx"])
    else:
        raise RuntimeError(
            f"Could not determine transaction count for block height {height}."
        )

    return block_time, tx_count


def compute_tx_count_daily(
    rpc: BitcoinRPC,
    start_date: date,
    end_date: date,
    height_margin: int,
...
def plot_obm_series(
    ...
    """
    Generate a line plot for the computed daily transaction-count series.
    ...
    ax.set_title( f"Open Bitcoin Metrics: Daily Transaction Count " f"({start_date} to {end_date})")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate the OBM daily Bitcoin transaction-count series."
    )
    ...
    parser.add_argument(
        "--output",
        default="obm_tx_count_daily.csv",
