#!/usr/bin/env python3
"""
compute_obm_fees_btc_daily.py

Generate the Open Bitcoin Metrics daily transaction-fees series:

    obm_fees_btc_daily

The script queries a local Bitcoin Core node through JSON-RPC, retrieves blocks
whose timestamps fall within the requested UTC date interval, computes total
transaction fees paid by non-coinbase transactions, aggregates them by UTC date,
...
Definition:
    ...
    transaction fee for transaction tx =
        sum(input values) - sum(output values)

    daily fees in BTC =
        sum of transaction fees over all non-coinbase transactions
        included in blocks assigned to that UTC day

Interpretation:
    This metric measures BTC transferred from users to miners as transaction
    fees. It excludes the block subsidy. Therefore, it is different from
    obm_miner_revenue_btc_daily, which includes both issuance and fees.

Important:
    This script requests getblock with verbosity 3. In Bitcoin Core, this can
    provide previous-output information for transaction inputs when the node has
    the required undo data. A non-pruned node is strongly recommended.

Assumptions:
    - Bitcoin Core is running and fully synchronized.
    - The RPC interface is accessible.
    - Dates are interpreted in UTC.
    - Historical blocks and undo data are available when reconstructing fees.
"""
...
def get_decoded_block_with_transactions_and_prevouts(
    rpc: BitcoinRPC,
    height: int,
) -> Dict[str, Any]:
    block_hash = rpc.call("getblockhash", [height])

    # Verbosity 3 returns decoded transactions and previous-output information
    # for inputs when available. This allows fee reconstruction without a
    # persistent local UTXO database.
    try:
        return rpc.call("getblock", [block_hash, 3])
    except RuntimeError as exc:
        raise RuntimeError(
            f"Could not retrieve block {height} with transaction prevout data. "
            "Fee computation requires either explicit fee fields or previous-output "
            "values. A non-pruned Bitcoin Core node with sufficient undo data is "
            "recommended."
        ) from exc
...
def tx_fee_sats_from_prevouts(tx: Dict[str, Any]) -> int:
    """
    Compute transaction fee as sum(inputs) - sum(outputs), using vin.prevout.
...
def tx_fee_sats(tx: Dict[str, Any]) -> int:
    ...
    If Bitcoin Core provides a fee field, use it. Otherwise compute the fee
    from previous-output values.
    """
    if "fee" in tx:
...
def block_fees_sats(block: Dict[str, Any]) -> int:
    """
    Compute total non-coinbase transaction fees in a block.
    """
    txs = block.get("tx")
    ...
    # Skip txs[0], the coinbase transaction.
    for tx in txs[1:]:
...
def compute_fees_btc_daily(
    ...
    for i, height in enumerate(range(scan_start_height, scan_end_height + 1), start=1):
        block = get_decoded_block_with_transactions_and_prevouts(rpc, height)
        block_time = int(block["time"])
        block_date = timestamp_to_utc_date(block_time)

        if start_date <= block_date <= end_date:
            fees_sats = block_fees_sats(block)
            fees_by_date[block_date] += fees_sats
...
def write_obm_csv(
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for d in sorted(fees_by_date):
            writer.writerow(
                {
                    "date": d.isoformat(),
                    "series_id": SERIES_ID,
                    "value": sats_to_btc_string(fees_by_date[d]),
                    "unit": UNIT,
                    "frequency": FREQUENCY,
...
def plot_obm_series(
    ...
    """
    Generate a line plot for the computed daily transaction-fees series.
    ...
    dates = sorted(fees_by_date)
    values = [
        float(Decimal(fees_by_date[d]) / SATOSHIS_PER_BTC)
        for d in dates
    ]
    ...
    ax.set_title(
        f"Open Bitcoin Metrics: Daily Transaction Fees in BTC "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate the OBM daily BTC-denominated transaction-fees series."
    )
    ...
    parser.add_argument(
        "--output",
        default="obm_fees_btc_daily.csv",
...
def main() -> int:
    ...
    try:
        ...
        fees_by_date = compute_fees_btc_daily(
            rpc=rpc,
        ...
        write_obm_csv(
            ...
            fees_by_date=fees_by_date,
            release_version=args.release_version,
        )

        print(
            f"Wrote {len(fees_by_date)} daily observations to {output_path}",
            file=sys.stderr,
        )
