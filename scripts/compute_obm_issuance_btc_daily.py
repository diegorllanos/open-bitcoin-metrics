...
Generate the Open Bitcoin Metrics daily realized Bitcoin issuance series:

    obm_issuance_btc_daily

The script queries a local Bitcoin Core node through JSON-RPC, computes actual
realized Bitcoin issuance per block, aggregates issuance by UTC date, and
...
Important:
    This script computes actual realized issuance, not theoretical scheduled
    issuance. For each block:

        realized issuance = coinbase transaction outputs - transaction fees

    This requires transaction fee information. The script therefore requests
    decoded block data with transaction and prevout information.

Assumptions:
    - Bitcoin Core is running and fully synchronized.
    - The RPC interface is accessible.
    - Dates are interpreted in UTC.
    - The node has access to the historical block and undo data needed to
      retrieve prevout information for historical fee reconstruction.
"""
...
SERIES_ID = "obm_issuance_btc_daily"
UNIT = "BTC"
FREQUENCY = "daily"
...
def tx_fee_sats_from_prevouts(tx: Dict[str, Any]) -> int:
    """
    Compute the fee of a non-coinbase transaction using prevout data.

    This requires getblock verbosity 3 or equivalent data containing
    vin[*].prevout.value.
    """
    input_total = 0
    output_total = sum_vout_sats(tx)

    for vin in tx.get("vin", []):
        if "coinbase" in vin:
            raise ValueError("Coinbase transactions do not have ordinary fees.")

        prevout = vin.get("prevout")
        if prevout is None or "value" not in prevout:
            txid = tx.get("txid", "<unknown txid>")
            raise ValueError(
                f"Missing prevout value while computing fee for tx {txid}. "
                "Run this script on a non-pruned node with undo data available, "
                "or use a Bitcoin Core getblock verbosity level that exposes prevouts."
            )

        input_total += btc_to_sats(prevout["value"])

    fee = input_total - output_total

    if fee < 0:
        txid = tx.get("txid", "<unknown txid>")
        raise ValueError(f"Computed negative fee for tx {txid}.")

    return fee


def tx_fee_sats(tx: Dict[str, Any]) -> int:
    """
    Return the fee of a non-coinbase transaction in satoshis.

    Prefer the fee field if Bitcoin Core provides it. Otherwise compute the fee
    from prevout values.
    """
    if "fee" in tx:
        return btc_to_sats(tx["fee"])

    return tx_fee_sats_from_prevouts(tx)


def block_realized_issuance_sats(block: Dict[str, Any]) -> int:
    """
    Compute actual realized issuance for a block.

        actual issuance = coinbase output value - transaction fees

    The coinbase output value includes both newly created coins and fees paid by
    transactions in the block. Fees are not new supply, so they must be removed.
    """
    txs = block.get("tx")
    ...
    coinbase_tx = txs[0]
    coinbase_output_total = sum_vout_sats(coinbase_tx)

    total_fees = 0
    for tx in txs[1:]:
        total_fees += tx_fee_sats(tx)

    issuance = coinbase_output_total - total_fees

    if issuance < 0:
        block_hash = block.get("hash", "<unknown block hash>")
        raise ValueError(f"Computed negative realized issuance for block {block_hash}.")

    return issuance


def get_decoded_block_with_transactions(rpc: BitcoinRPC, height: int) -> Dict[str, Any]:
    block_hash = rpc.call("getblockhash", [height])

    try:
        return rpc.call("getblock", [block_hash, 3])
    except RuntimeError as exc:
        raise RuntimeError(
            f"Could not retrieve block {height} with transaction prevout data. "
            "For actual issuance reconstruction, this script needs sufficient data "
            "to compute transaction fees. A non-pruned Bitcoin Core node is "
            "recommended."
        ) from exc
...
def compute_issuance_btc_daily(
    rpc: BitcoinRPC,
    start_date: date,
    end_date: date,
    height_margin: int,
    ...
    """
    Return daily realized issuance in satoshis for each requested date.
    """
    ...
    if end_ts > tip_time:
        raise ValueError(
            "The requested end_date extends beyond the current chain tip timestamp. "
            "Choose an end_date that is complete according to your node tip, "
            "typically yesterday when running daily updates."
        )
    ...
    issuance_by_date: Dict[date, int] = {
    ...
    for i, height in enumerate(range(scan_start_height, scan_end_height + 1), start=1):
        ...
        if start_date <= block_date <= end_date:
            issuance_sats = block_realized_issuance_sats(block)
            issuance_by_date[block_date] += issuance_sats
...
def write_obm_csv(
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for d in sorted(issuance_by_date):
            writer.writerow(
                {
                    "date": d.isoformat(),
                    "series_id": SERIES_ID,
                    "value": sats_to_btc_string(issuance_by_date[d]),
                    "unit": UNIT,
                    "frequency": FREQUENCY,
...
def plot_obm_series(
    ...
    """
    Generate a line plot for the computed daily realized Bitcoin issuance series.
    ...
    dates = sorted(issuance_by_date)
    values = [float(Decimal(issuance_by_date[d]) / SATOSHIS_PER_BTC) for d in dates]
    ...
    ax.set_title(
        f"Open Bitcoin Metrics: Daily Realized Bitcoin Issuance "
        f"({start_date} to {end_date})"
    )
    ax.set_xlabel("Date")
    ax.set_ylabel("BTC issued per day")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate the OBM daily realized Bitcoin issuance series."
    )
    ...
    parser.add_argument(
        "--output",
        default="obm_issuance_btc_daily.csv",
...
def main() -> int:
    ...
    try:
        ...
        issuance_by_date = compute_issuance_btc_daily(
            rpc=rpc,
        ...
        write_obm_csv(
            ...
            issuance_by_date=issuance_by_date,
            release_version=args.release_version,
        )

        print(
            f"Wrote {len(issuance_by_date)} daily observations to {output_path}",
            file=sys.stderr,
        )
