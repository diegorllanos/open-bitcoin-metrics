...
SERIES_ID = "obm_cdd_per_supply_days_daily"
...
getcontext().prec = 40
SupplyAdjustedCddValue = Optional[Decimal]
...
def infer_date_interval(
    cdd_rows: Dict[date, ObmRow],
    supply_rows: Dict[date, ObmRow],
    start_date: Optional[date],
    end_date: Optional[date],
) -> tuple[date, date]:
    common_start = max(min(cdd_rows), min(supply_rows))
    common_end = min(max(cdd_rows), max(supply_rows))
...
def infer_release_version(
    ...
    for d in daterange(start_date, end_date):
        release_versions.add(cdd_rows[d].release_version)
        release_versions.add(supply_rows[d].release_version)
...
def compute_cdd_supply_adjusted(
    cdd_rows: Dict[date, ObmRow],
    supply_rows: Dict[date, ObmRow],
    start_date: date,
    end_date: date,
) -> Dict[date, SupplyAdjustedCddValue]:
    output: Dict[date, SupplyAdjustedCddValue] = {}

    for d in daterange(start_date, end_date):
        cdd = cdd_rows[d].value
        supply = supply_rows[d].value

        if cdd < 0:
            raise ValueError(f"Negative CDD value found for {d.isoformat()}.")

        if supply < 0:
            raise ValueError(f"Negative supply value found for {d.isoformat()}.")
...
def write_obm_csv(
    output_path: Path,
    value_by_date: Dict[date, SupplyAdjustedCddValue],
    release_version: str,
) -> None:
...
def plot_obm_series(
    ...
    dates = [
        d for d in sorted(value_by_date)
        if value_by_date[d] is not None
    ]
    values = [float(value_by_date[d]) for d in dates]

    if not dates:
        raise ValueError("No defined supply-adjusted CDD values are available to plot.")
    ...
    start = min(value_by_date).isoformat()
    end = max(value_by_date).isoformat()

    ax.set_title(
        f"Open Bitcoin Metrics: Supply-Adjusted Coin Days Destroyed "
        f"({start} to {end})"
    )
    ax.set_xlabel("Date")
    ax.set_ylabel("Days")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the OBM daily supply-adjusted Coin Days Destroyed series "
            "from obm_cdd_btcxdays_daily and obm_supply_btc_daily CSV files."
        )
    )

    parser.add_argument(
        "cdd_csv",
        help="Input CSV file for obm_cdd_btcxdays_daily.",
    )

    parser.add_argument(
        "supply_csv",
        help="Input CSV file for obm_supply_btc_daily.",
    )
    ...
    parser.add_argument(
        "--output",
        default="obm_cdd_per_supply_days_daily.csv",
...
def main() -> int:
    ...
    try:
        cdd_rows = read_obm_csv(
            input_path=Path(args.cdd_csv),
            expected_series_id=CDD_SERIES_ID,
            expected_unit=CDD_UNIT,
            expected_frequency=FREQUENCY,
        )

        supply_rows = read_obm_csv(
            input_path=Path(args.supply_csv),
        ...
        value_by_date = compute_cdd_supply_adjusted(
            cdd_rows=cdd_rows,
            supply_rows=supply_rows,
            start_date=start_date,
            end_date=end_date,
        ...
        print(
            f"Wrote {len(value_by_date)} daily observations to {output_path}",
            file=sys.stderr,
        )
